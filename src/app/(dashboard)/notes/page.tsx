...
type NoteColor = 'default' | 'yellow' | 'green' | 'blue' | 'purple' | 'pink';
...
export default function NotesPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const [foldersRes, notesRes] = await Promise.all([
        supabase.from('note_folders').select('*').eq('tenant_id', tid).order('sort_order'),
        supabase.from('notes').select('*, profiles:author_id(full_name)').eq('tenant_id', tid).order('updated_at', { ascending: false }),
      ]);

      setFolders(foldersRes.data || []);
      setNotesList((notesRes.data as unknown as Note[]) || []);
      setLoading(false);
    }
    fetchData();
  }, []);
    ...
    if (!error) {
      const updated = notesList.map(n =>
        n.id === selectedNote.id
          ? { ...n, title: editTitle, content: editContent, folder_id: editFolder || null, color: editColor, updated_at: new Date().toISOString() }
          : n
      );
      setNotesList(updated);
      setSelectedNote(updated.find(n => n.id === selectedNote.id)!);
    }
    setIsEditing(false);
  }
  ...
  async function togglePin(noteId: string) {
    ...
    if (!note) return;
    const supabase = createClient();
    await supabase.from('notes').update({ is_pinned: !note.is_pinned }).eq('id', noteId);
    const updated = notesList.map(n =>
      n.id === noteId ? { ...n, is_pinned: !n.is_pinned, updated_at: new Date().toISOString() } : n
    );
    setNotesList(updated);
    if (selectedNote?.id === noteId) setSelectedNote(updated.find(n => n.id === noteId)!);
  }
  ...
  if (loading) {
    return (
      <div className="h-[calc(100vh-3.5rem)] lg:h-[calc(100vh-2rem)] flex items-center justify-center -m-4 sm:-m-6 lg:-m-8 bg-[var(--card)] rounded-xl border border-[var(--border)]">
  ...
  const sidebarPanel = (
    <div className="flex flex-col h-full">
      <div className="p-3 border-b border-[var(--border)]">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 text-sm border border-[var(--border)] rounded-lg bg-[var(--card)] focus:outline-none focus:ring-2 focus:ring-[var(--accent)]/30"
          />
        </div>
      </div>
      ...
      <div className="p-3 border-b border-[var(--border)]">
        ...
        {folders.map(folder => {
          const count = notesList.filter(n => n.folder_id === folder.id).length;
          return (
            <button
              key={folder.id}
            ...
            >
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-[var(--muted)] mt-1">{formatDate(note.updated_at)}</p>
                </div>
  ...
  return (
    <div className="h-[calc(100vh-3.5rem)] lg:h-[calc(100vh-2rem)] flex flex-col -m-4 sm:-m-6 lg:-m-8 bg-[var(--card)] rounded-xl border border-[var(--border)] shadow-sm overflow-hidden">
      {/* Header */}
      <div className="flex items-center justify-between p-4 sm:p-6 border-b border-[var(--border)] bg-white flex-shrink-0">
        ...
        <button
          ...
          className="flex items-center gap-2 px-3 py-2 sm:px-4 bg-[var(--accent)] text-white rounded-lg hover:bg-[var(--accent-hover)] transition-colors text-sm font-medium"
        >
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        <div className={`
          ...
          w-80 bg-[var(--card)] border-r border-[var(--border)] h-full
          ...
          {(isEditing && (showNewNote || selectedNote)) ? (
            <div className="max-w-3xl mx-auto p-4 sm:p-8">
              <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="px-4 py-1.5 text-sm bg-[var(--accent)] text-white rounded-lg hover:bg-[var(--accent-hover)] transition-colors font-medium"
                  >
              ...
              <select
                ...
                className="mb-4 text-xs px-2 py-1 border border-[var(--border)] rounded-md bg-[var(--card)] text-[var(--muted)] focus:outline-none"
              >
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-2 flex-wrap">
                    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${colorMap[selectedNote.color]?.bg || ''} ${colorMap[selectedNote.color]?.border || ''} border`}>
                      ...
                      {getFolderName(selectedNote.folder_id)}
                    </span>
                  ...
                  <div className="flex items-center gap-3 mt-2 text-xs text-[var(--muted)] flex-wrap">
                    {selectedNote.profiles?.full_name && <span>{selectedNote.profiles.full_name}</span>}
                    <span>Updated {formatDate(selectedNote.updated_at)} at {formatTime(selectedNote.updated_at)}</span>
                  </div>
                </div>
                <div className="flex items-center gap-1 ml-4 flex-shrink-0">
                  <button
                    onClick={() => togglePin(selectedNote.id)}
                    className={`p-2 rounded-lg transition-colors ${selectedNote.is_pinned ? 'text-[var(--accent)] bg-[var(--accent)]/10' : 'text-[var(--muted)] hover:text-[var(--foreground)] hover:bg-[var(--muted)]/20'}`}
