...
type SOPStatus = 'draft' | 'active' | 'under_review' | 'archived';
type SOPCategory = 'Client Onboarding' | 'Web Design' | 'SEO' | 'Social Media' | 'Content' | 'Development' | 'Internal' | 'Sales';
type SOPStep = { title: string; description: string; isOptional?: boolean };
...
const statuses: SOPStatus[] = ['draft', 'active', 'under_review', 'archived'];
...
export default function SOPsPage() {
  const [sopsList, setSopsList] = useState<SOP[]>([]);
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      setSopsList((data as unknown as SOP[]) || []);
      setLoading(false);
    }
    fetchData();
  }, []);
    ...
    if (editingSop) {
      ...
      if (!error) {
        const updated = sopsList.map(s =>
          s.id === editingSop.id
            ? { ...s, title: formTitle, description: formDescription, category: formCategory, status: formStatus, steps: cleanSteps, updated_at: new Date().toISOString() }
            : s
        );
        setSopsList(updated);
      ...
      if (!error && data) {
        setSopsList([data as unknown as SOP, ...sopsList]);
      }
    }
    setShowModal(false);
  }
    ...
    return (
      <div className="space-y-6 p-4 sm:p-6 lg:p-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {[1,2,3,4].map(i => <div key={i} className="h-20 bg-[var(--card)] rounded-xl border border-[var(--border)] animate-pulse" />)}
        </div>
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {[1,2,3].map(i => <div key={i} className="h-40 bg-[var(--card)] rounded-xl border border-[var(--border)] animate-pulse" />)}
        </div>
      </div>
    );
  }
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-3 flex-wrap">
                  ...
                  <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${statusStyles[selectedSop.status]}`}>
                    {statusLabels[selectedSop.status]}
                  </span>
                ...
                <div className="flex items-center gap-4 mt-4 text-xs text-[var(--muted)] flex-wrap">
                  {selectedSop.profiles?.full_name && <span>By {selectedSop.profiles.full_name}</span>}
                  <span>Updated {formatDate(selectedSop.updated_at)}</span>
                </div>
              </div>
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        ...
        <button
          onClick={openNewSop}
          className="flex items-center gap-2 px-4 py-2 bg-[var(--accent)] text-white rounded-lg hover:bg-[var(--accent-hover)] transition-colors text-sm font-medium self-start sm:self-auto"
        >
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {statuses.map(status => {
          ...
          return (
            <div key={status} className="bg-white rounded-xl border border-[var(--border)] p-4">
              <p className="text-xs text-[var(--muted)] mb-1">{statusLabels[status]}</p>
              ...
              {categorySops.map(sop => (
                ...
                >
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs text-[var(--muted)]">{formatDate(sop.updated_at)}</span>
                  </div>
                </button>
              ))}
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center p-4 pt-[10vh] bg-black/40 overflow-y-auto" onClick={() => setShowModal(false)}>
          <div className="bg-white rounded-2xl border border-[var(--border)] shadow-xl w-full max-w-2xl my-4" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-6 space-y-4 max-h-[60vh] overflow-y-auto">
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button type="button" onClick={addStep} className="text-xs text-[var(--accent)] hover:underline font-medium">
                    + Add Step
                  </button>
                </div>
                <div className="space-y-3">
                  {formSteps.map((step, index) => (
                    <div key={index} className="flex gap-3 p-3 rounded-lg border border-[var(--border)] bg-slate-50">
                      ...
                      {formSteps.length > 1 && (
                        <button type="button" onClick={() => removeStep(index)} className="flex-shrink-0 p-1 text-[var(--muted)] hover:text-red-500 transition-colors mt-1">
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="p-6 border-t border-[var(--border)] flex items-center justify-end gap-3">
              ...
              <button
                onClick={saveSop}
                className="px-4 py-2 text-sm bg-[var(--accent)] text-white rounded-lg hover:bg-[var(--accent-hover)] transition-colors font-medium"
              >
